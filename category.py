...
def update_category(id_entry, name_entry, description_text, treeview, clear_func):
    ...
    if id_val == str(old_id) and name_val == old_name and description_val == old_description:
...
def add_category(id_val, name_val, description_val, treeview, clear_func):
    ...
    try:
        ...
        cursor.execute('INSERT INTO category_data VALUES(%s, %s, %s)', (id_val, name_val, description_val))
        connection.commit()
...
def category_form(window):
    ...
    heading_label = Label(category_frame, text='مدیریت دسته بندی محصولات',
    ...
    export_button = Button(export_frame, text='📊 خروجی CSV',
                           font=('fonts/Persian-Yekan.ttf', 12),
                           width=15, fg='white', bg='#4b39e9',
    ...
    id_label = Label(details_frame, text='شناسه',
                     font=('fonts/Persian-Yekan.ttf', 14, 'bold'),
                     bg='white', anchor='e', width=15)  # anchor='e' برای راست‌چین
    id_label.grid(row=0, column=1, padx=(0, 20), pady=10, sticky='e')  # padx=(0, 20): فاصله از راست

    id_entry = Entry(details_frame, font=('fonts/Persian-Yekan.ttf', 12),
                     bg='lightblue', width=25, justify=RIGHT)
    id_entry.grid(row=0, column=0, padx=(20, 10), pady=10, sticky='ew')

    category_name_label = Label(details_frame, text='نام دسته بندی',
                                font=('fonts/Persian-Yekan.ttf', 14, 'bold'),
                                bg='white', anchor='e', width=15)
    category_name_label.grid(row=1, column=1, padx=(0, 20), pady=10, sticky='e')

    category_name_entry = Entry(details_frame, font=('fonts/Persian-Yekan.ttf', 12),
                                bg='lightblue', width=25, justify=RIGHT)
    category_name_entry.grid(row=1, column=0, padx=(20, 10), pady=10, sticky='ew')

    description_label = Label(details_frame, text='توضیحات',
                              font=('fonts/Persian-Yekan.ttf', 14, 'bold'),
                              bg='white', anchor='ne', width=15)  # anchor='ne' برای بالا-راست
    ...
    add_button = Button(button_frame, text='افزودن',
                        ...
                        command=lambda: add_category(id_entry.get(), category_name_entry.get(),
                                                     description_text.get(1.0, END).strip(), treeview, clear_func))
    ...
    treeview.bind('<<TreeviewSelect>>',
                  lambda event: select_data(event, treeview, id_entry,
                                            category_name_entry, description_text))